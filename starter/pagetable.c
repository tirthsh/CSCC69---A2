...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		//get the victim page based on the frame recieved from your evict algorithm
		pgtbl_entry_t *victim_page = coremap[frame].pte;

		//check if victim page is holding a frame and the page is dirty (altered)
		if(victim_page->frame & (PG_DIRTY)){
			int swapped_page_out = swap_pageout(frame, victim_page->swap_off);
			assert(swapped_page_out != INVALID_SWAP);
			//update the victim pages swap off value
			victim_page->swap_off = swapped_page_out;
			//successfully evicted a dirty page
			evict_dirty_count++;
			} else { //if the page is not dirty then we dont need to add anything to the swap file
				evict_clean_count++;
			}
		//now update all the victim frame values because now it is evicted
		//set this bit to TRUE because the page is successfully evicted
		victim_page->frame |= PG_ONSWAP;
		//set frame to invalid because frame is now on disk
...
char *find_physpage(addr_t vaddr, char type) {
	...
	unsigned pg_idx = PGTBL_INDEX(vaddr); //index for page table 

	// IMPLEMENTATION NEEDED
	// Use top-level page directory to get pointer to 2nd-level page table
	//(void)idx; // To keep compiler happy - remove when you have a real use.

	//first check if idx page table is in page directory
	//if not, create one in second level 

	if (pgdir[idx].pde == 0){
		//create a new page table called idx in second level
		//use helper function
		pgdir[idx] = init_second_level();
	}

	// Use vaddr to get index into 2nd-level page table and initialize 'p'
	//create new page table entry in the page directory 
	//initialize the last 12 bits of the page table entry to 0 using bit 
	//masking technique
	pgtbl_entry_t *page_table_entry = (pgtbl_entry_t *) (pgdir[idx].pde & PAGE_MASK);

	//a pointer pointing to the page table 
	p = &page_table_entry[pg_idx];

	// Check if p is valid or not, on swap or not, and handle appropriately
	//check if the pointing to the frame is valid or not
	//if valid => in main memory
	//if invalid => in swap

	if ((p->frame & PG_VALID) == 0){ //not in main memory
		miss_count++;
		int frame_num = allocate_frame(p);

		if((p->frame & PG_ONSWAP) == 0) { //not in swap either


			//shift address to 12 bits to the left so you get the virtual
			//addr
			p->frame = frame_num << PAGE_SHIFT;
			init_frame(frame_num, vaddr);


			//techinally its dirty since it doesnt exist in the swap
			p->frame |= PG_DIRTY;
		}
		else{
			//otherwise on swap file
			//get frame from swap file
			int swapped_in = swap_pagein(frame_num, p->swap_off);
			//check for errors
			assert(swapped_in == 0);
	...
	//otherwise in the main memory
	else{
		hit_count++;
	}


	//if type is S or M, set frame to dirty since it needs to be put on 
	//the swap
	if((type == 'S') || (type == 'M')){
		p->frame |= PG_DIRTY;
	}
	...
	//since its in memory now, set it to valid
	p->frame |= PG_VALID;
	//set frame to ref since its referenced now
	p->frame |= PG_REF;
	ref_count++;
